...
void do_udp_config(void)
{
  ...
  case SOCK_UDP:
    if((len=getSn_RX_RSR(SOCK_CONFIG))>0)
    {
      ...
      if((RecvMsg.op[0]=='F') && (RecvMsg.op[1]=='I') && (RecvMsg.op[2]=='N') && (RecvMsg.op[3]=='D'))//FIND 搜索
      {
        RecvMsg = ConfigMsg;
      ...
      else if((RecvMsg.op[0]=='S') && (RecvMsg.op[1]=='E') && (RecvMsg.op[2]=='T') && (RecvMsg.op[3]=='T'))//SETT 设置
      {
				printf("SEET\r\n");
        ...
        {					
          ...
          write_config_to_flash(); //写入Flash
          reboot();
        }
      }
      else if((RecvMsg.op[0]=='F') && (RecvMsg.op[1]=='I') && (RecvMsg.op[2]=='R') && (RecvMsg.op[3]=='M'))//FIRM 检测到固件升级
      {
				printf("FIR\r\n");
